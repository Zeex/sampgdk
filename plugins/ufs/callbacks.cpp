...
class OnPlayerTakeDamage {
 public:
  OnPlayerTakeDamage(int playerid, int issuerid, float amount, int weaponid, int bodypart): playerid(playerid), issuerid(issuerid), amount(amount), weaponid(weaponid), bodypart(bodypart) {}
  bool operator()(Script *s) {
    amx_Push(s->amx(), bodypart);
...
class OnPlayerGiveDamage {
 public:
  OnPlayerGiveDamage(int playerid, int damagedid, float amount, int weaponid, int bodypart): playerid(playerid), damagedid(damagedid), amount(amount), weaponid(weaponid), bodypart(bodypart) {}
  bool operator()(Script *s) {
    amx_Push(s->amx(), bodypart);
    amx_Push(s->amx(), weaponid);
    amx_Push(s->amx(), amx_ftoc(amount));
...
class OnPlayerWeaponShot {
 public:
  OnPlayerWeaponShot(int playerid, int weaponid, int hittype, int hitid, float fX, float fY, float fZ): playerid(playerid), weaponid(weaponid), hittype(hittype), hitid(hitid), fX(fX), fY(fY), fZ(fZ) {}
  bool operator()(Script *s) {
    amx_Push(s->amx(), amx_ftoc(fZ));
    amx_Push(s->amx(), amx_ftoc(fY));
    amx_Push(s->amx(), amx_ftoc(fX));
    amx_Push(s->amx(), hitid);
    amx_Push(s->amx(), hittype);
    amx_Push(s->amx(), weaponid);
    amx_Push(s->amx(), playerid);
...
PLUGIN_EXPORT bool PLUGIN_CALL OnPlayerTakeDamage(int playerid, int issuerid, float amount, int weaponid, int bodypart) {
  using namespace ufs;
  return UFS::Instance().ForEachScript(
    ufs::OnPlayerTakeDamage(playerid, issuerid, amount, weaponid, bodypart), true);
}

PLUGIN_EXPORT bool PLUGIN_CALL OnPlayerGiveDamage(int playerid, int damagedid, float amount, int weaponid, int bodypart) {
  using namespace ufs;
  return UFS::Instance().ForEachScript(
    ufs::OnPlayerGiveDamage(playerid, damagedid, amount, weaponid, bodypart), true);
}
...
PLUGIN_EXPORT bool PLUGIN_CALL OnPlayerWeaponShot(int playerid, int weaponid, int hittype, int hitid, float fX, float fY, float fZ) {
  using namespace ufs;
  return UFS::Instance().ForEachScript(
    ufs::OnPlayerWeaponShot(playerid, weaponid, hittype, hitid, fX, fY, fZ), true);
}
